  ...
  for (const subject of state.subjects) {
    for (const chapter of subject.chapters) {
      ...
      for (const item of dates) {
        if (item.rule.name === "Chapter Start") {
          continue;
        }
...
async function deleteSubject(subjectId) {
  ...
  for (const chapter of subject.chapters) {
    await deleteChapterFiles(chapter.id);
  }
...
async function moveChapter(chapterId) {
  ...
  const select =
    document.getElementById("moveSubject");

  if (!select) return;

  const target = findSubject(select.value);
...
async function openNotes(chapterId) {
  ...
  const input =
    document.getElementById("noteFileInput");
...
function getStoredBlob(file) {
  ...
  /*
    Some browsers may return the IndexedDB
    Blob as a Blob-like object.

    Rebuilding it here makes the MIME type
    explicit, especially for PDF.
  */
...
async function storeNoteFile(chapterId, file) {
  ...
  try {
    ...
    /*
      Android file picker sometimes gives PDF
...
async function viewStoredFile(fileId) {
  try {
    ...
    /*
      IMPORTANT:
      PDF is opened in a separate browser tab
      instead of iframe.

      This is much more reliable on Android
      Chrome/PWA than iframe PDF rendering.
    */

    if (type === "pdf") {
      await openPDFFile(file, blob);
      return;
    }
...
async function openPDFFile(file, blob) {
  try {
    /*
      Re-create the PDF blob with an explicit
    ...
    */
    ...
    /*
      Try a new browser tab first.
      Android Chrome normally handles PDF here.
    */
    ...
    /*
      If popup is blocked, show an in-app
      PDF viewer as fallback.
    */
...
function showPDFFallback(file, url) {
  const title =
    escapeHTML(
      file.name || "PDF"
    );
...
function showFileViewer(file, type, url) {
  const title =
    escapeHTML(
      file.name || "File"
    );
...
async function saveRevisionSystem() {
  ...
  for (
    let i = 0;
    i < state.revisionSystem.length;
    i++
  ) {
    const name =
      document
        .getElementById(`ruleName${i}`)
        ?.value
        .trim();
    ...
    rows.push({
      id:
        state.revisionSystem[i].id ||
        uid(),
      name,
      day,
      final:
        i === state.revisionSystem.length - 1
    });
  }
