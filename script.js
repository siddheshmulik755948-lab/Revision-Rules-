...
function chapterProgress(chapter) {
  const total = totalRevisionCount();
  ...
  return Math.min(
    100,
    Math.round(
      (completedCount(chapter) / total) * 100
    )
  );
}
...
function progressBarHTML(progress, extraClass = "") {
  const safe =
    Math.max(
      0,
      Math.min(
                ...
                <div class="muted">
                  ${
                    exam.days === 0
                      ? "Exam is today!"
                      : exam.days > 0
                        ? `${formatDate(state.settings.examDate)} • ${exam.label}`
                        : `${formatDate(state.settings.examDate)} • Date passed`
                  }
                </div>
              `
              : `
        ...
        <div
          ...
          ${
            state.subjects.length
              ...
              : `
                <div class="empty">
                  Add subjects to see subject-wise progress.
                </div>
              `
          }
  ...
  return `
    <div class="card chapter-card">
      ...
      <div class="row">
        ...
        <div class="chapter-progress-number">
          ${progress}%
        </div>
    ...
    <section>
      ...
      <div class="card exam-settings-card">
        ...
        <p class="muted" style="margin-top:7px">
          Choose your exam date. You can change it anytime.
        </p>
...
async function saveExamDate() {
  ...
  if (!value) {
    toast("Please select an exam date.");
    return;
  }
