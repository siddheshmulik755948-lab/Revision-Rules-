...
function getRevisionDates(
  ...
  return state.revisionSystem.map(
    rule => {
      const date =
        dateFromISO(
          chapter.startDate
...
/*
   ...
   We create a fresh Blob URL
   only when the file is actually
   being viewed.

   The URL is kept alive until
...
function renderNoteFile(
  ...
  /*
     ...
     The Blob URL is created only
     after pressing Open.

     This avoids stale / invalid
     object URLs in Android PWA.
  */
...
function createFileBlob(
  ...
  /*
     ...
     stored a Blob directly.

     New records also contain
     original MIME type.
  */
...
function showModal(
  ...
  /*
     Clean any previous file URL
     when opening a completely
     different modal.
  */
