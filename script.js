...
const defaultRevisionSystem = [
  { id: crypto.randomUUID(), name: "Chapter Start", day: 1, final: false },
  { id: crypto.randomUUID(), name: "Revision 1", day: 2, final: false },
  { id: crypto.randomUUID(), name: "Revision 2", day: 4, final: false },
  { id: crypto.randomUUID(), name: "Revision 3", day: 8, final: false },
  { id: crypto.randomUUID(), name: "Revision 4", day: 16, final: false },
  { id: crypto.randomUUID(), name: "Revision 5", day: 32, final: false },
  { id: crypto.randomUUID(), name: "Final Revision", day: 65, final: true }
];
...
async function loadState() {
  ...
  if (!Array.isArray(state.revisionSystem)) {
    state.revisionSystem = defaultRevisionSystem;
  }
}
...
function formatDate(value) {
  ...
  return dateFromISO(value).toLocaleDateString("en-IN", {
    day: "2-digit",
    month: "short",
    year: "numeric"
  });
...
function getRevisionItems() {
  ...
  for (const subject of state.subjects) {
    for (const chapter of subject.chapters) {
      ...
      dates.forEach(item => {
        if (item.rule.name === "Chapter Start") return;

        const completion = chapter.revisions?.[item.rule.id];
...
function dueTodayItems() {
  ...
  return getRevisionItems().filter(item => {
    return item.date === today && !item.completion?.submitted;
  });
}
...
function completedCount(chapter) {
  if (!chapter.revisions) return 0;

  return Object.values(chapter.revisions)
    .filter(v => v && v.submitted).length;
}
...
function renderHome() {
  ...
  const totalRevisions =
    totalChapters *
    Math.max(0, state.revisionSystem.length - 1);

  const progress =
    totalRevisions === 0
      ? 0
      : Math.round((completed / totalRevisions) * 100);

  return `
    <section>
      <h1 class="page-title">Good day 👋</h1>
      ...
      <div class="card hero">
        <div class="row">
          <div>
            ...
            <div class="stat-number">${Math.min(progress, 100)}%</div>
          </div>
      ...
      <div class="grid grid-2" style="margin-top:14px">
        ...
        <div class="card">
          ...
          <div class="stat-label">Incomplete Revisions</div>
        </div>
      ...
      <div class="card" style="margin-top:14px">
        <div class="row">
          <div>
            <div class="title">Today's Revisions</div>
            <div class="muted">${today} pending</div>
          </div>

          <button class="btn small" onclick="goPage('today')">
            Open
          </button>
        </div>
      </div>
    ...
    <section>
      <div class="row">
        <div>
          ...
          <p class="page-subtitle">
            Add as many subjects and chapters as you need.
          </p>
        </div>

        <button class="btn" onclick="openSubjectModal()">
          + Subject
        </button>
      </div>
...
function renderChapterCard(subject, chapter) {
  ...
  const total = Math.max(0, state.revisionSystem.length - 1);

  return `
    <div class="card chapter-card">
      <div class="row">
        <div>
          <div class="title">${escapeHTML(chapter.name)}</div>
          <div class="muted">
            Started: ${formatDate(chapter.startDate)}
          </div>

          <div class="badge" style="margin-top:7px">
        ...
        <div class="actions">
          <button class="btn small secondary"
            onclick="openChapterModal('${subject.id}','${chapter.id}')">
            Edit
          </button>
...
function renderToday() {
  ...
  return `
    <section>
      <h1 class="page-title">Today's Revisions</h1>
      <p class="page-subtitle">
        Complete and submit each revision to make it final.
      </p>
      ...
      <div class="card" style="margin-top:15px">
        <div class="row">
          <div>
            <div class="title">Incomplete Revisions</div>
            <div class="muted">
              Revisions that were missed
            </div>
          </div>

          <span class="badge danger">${incomplete.length}</span>
        </div>

        <div style="margin-top:12px">
          ${
            incomplete.length
              ? incomplete.map(renderRevisionCard).join("")
              : `<div class="empty">
...
function renderRevisionCard(item) {
  ...
  const checked = !!item.completion?.checked;
  ...
  return `
    <div class="card revision-card">
      ...
      <div style="margin-top:10px" class="muted">
        Scheduled: <strong>${formatDate(item.date)}</strong>
      </div>

      ${
        lateDays > 0
          ? `
            <div class="late" style="margin-top:5px">
              ${lateDays} day(s) late
            </div>
          `
          : ""
...
async function toggleRevisionCheck(chapterId, ruleId) {
  ...
  if (chapter.revisions[ruleId]?.submitted) {
    toast("This revision is already final.");
    return;
  }

  const old = chapter.revisions[ruleId] || {};
...
async function confirmRevision(chapterId, ruleId) {
  const found = findChapter(chapterId);
  ...
  if (!chapter.revisions?.[ruleId]?.checked) {
  ...
  chapter.revisions[ruleId].submittedAt = todayISO();
...
function renderPlanner() {
  return `
    <section>
      <h1 class="page-title">Date Planner</h1>
      <p class="page-subtitle">
        Enter any date to see which revisions are scheduled.
      </p>
      ...
      <div id="plannerResults" style="margin-top:14px"></div>
    </section>
  `;
}
...
function showPlannerDate() {
  ...
  const results = getRevisionItems().filter(
    item => item.date === date
  );
  ...
  if (!results.length) {
    container.innerHTML = `
      <div class="card empty">
        No revision scheduled for ${formatDate(date)}.
      </div>
    `;
    return;
  }
...
function openChapterModal(subjectId, chapterId = "") {
  const subject = findSubject(subjectId);
  const chapter = chapterId
    ? findChapter(chapterId)?.chapter
    : null;
  ...
  showModal(`
    ...
    <div class="form-group">
      <label>Study Start Date (Day 1)</label>
      <input
        type="date"
        id="chapterDate"
    ...
    <div class="actions">
      ...
      <button class="btn"
        onclick="saveChapter('${subjectId}','${chapterId}')">
        Save
      </button>
    </div>
  `);
...
async function saveChapter(subjectId, chapterId) {
  const subject = findSubject(subjectId);
  ...
  if (!name || !date) {
    toast("Enter chapter name and date.");
    return;
  }

  if (chapterId) {
    const chapter = findChapter(chapterId).chapter;

    chapter.name = name;
    chapter.startDate = date;
  } else {
...
function openMoveModal(chapterId) {
  ...
  const options = state.subjects
    .filter(s => s.id !== found.subject.id)
    .map(s =>
      `<option value="${s.id}">
        ${escapeHTML(s.name)}
       </option>`
...
async function moveChapter(chapterId) {
  ...
  const targetId =
    document.getElementById("moveSubject").value;

  const target = findSubject(targetId);
  ...
  target.chapters.push(found.chapter);
...
/*
  IMPORTANT:
  Notes are stored as actual File/Blob objects in IndexedDB.
  They are NOT external links.

  Therefore:
  - Flight Mode works.
  - Original gallery/file copy can be deleted.
  - App's stored copy remains until the user deletes it.
*/
...
async function openNotes(chapterId) {
  ...
  const files = await dbGetAll(FILE_STORE);
    ...
    <div id="notesContainer">
      ${
        chapterFiles.length
          ? `
            <div class="file-grid">
              ${chapterFiles.map(renderNoteFile).join("")}
            </div>
          `
          : `
            <div class="empty">
  ...
  document
    ...
    .addEventListener("change", async event => {
      ...
      for (const file of selectedFiles) {
        await storeNoteFile(chapterId, file);
      }
...
async function storeNoteFile(chapterId, file) {
  const storedFile = {
    id: uid(),
    chapterId,
    name: file.name,
  ...
  await dbPut(FILE_STORE, storedFile);
}

async function viewStoredFile(fileId) {
  const file = await dbGet(FILE_STORE, fileId);
  ...
  if (!win) {
    toast("Allow pop-ups to open the file.");
    return;
  }
...
async function deleteNoteFile(fileId) {
  if (!confirm("Delete this note from the app?")) {
    return;
  }

  await dbDelete(FILE_STORE, fileId);
...
async function deleteChapterFiles(chapterId) {
  const files = await dbGetAll(FILE_STORE);

  for (const file of files) {
    if (file.chapterId === chapterId) {
      await dbDelete(FILE_STORE, file.id);
    }
  }
}
...
function renderSettings() {
  return `
    <section>
      ...
      <p class="page-subtitle">
        You control your revision system and data.
      </p>

      <div class="card">
        <div class="row">
          <div>
            <div class="title">Revision System</div>
            <div class="muted">
      ...
      <div class="card" style="margin-top:14px">
        ...
        <p class="muted" style="margin-top:7px">
          Your study data and uploaded Notes are stored
          locally on this device.
        </p>

        <div class="badge success" style="margin-top:10px">
          Offline Ready
        </div>
      </div>
    </section>
...
function openRevisionSystem() {
  showModal(`
    <h2>Revision System</h2>

    <p class="muted" style="margin-bottom:14px">
      Day 1 is the chapter start day.
      You can completely control the schedule.
    </p>

    <div id="revisionRules">
      ${state.revisionSystem.map((rule, index) => `
...
function addRevisionRuleRow() {
  state.revisionSystem.push({
    id: uid(),
    name: `Revision ${state.revisionSystem.length}`,
    day: Number(
...
function removeRevisionRuleRow() {
  if (state.revisionSystem.length <= 2) {
    toast("Keep at least Chapter Start + one revision.");
    return;
  }
...
async function saveRevisionSystem() {
  ...
  for (let i = 0; i < state.revisionSystem.length; i++) {
    const name =
      document.getElementById(`ruleName${i}`)?.value.trim();

    const day =
      Number(document.getElementById(`ruleDay${i}`)?.value);

    if (!name || !Number.isFinite(day) || day < 1) {
      toast("Check revision names and days.");
      return;
    }

    rows.push({
      id: state.revisionSystem[i].id || uid(),
      name,
      day,
      final: i === state.revisionSystem.length - 1
    });
  }
  ...
  showModal(`
    ...
    <div style="margin-top:15px">
      <label style="display:block;margin-bottom:10px">
        <input type="checkbox" id="deleteSubjects">
        Delete all Subjects & Chapters
      </label>

      <label style="display:block;margin-bottom:10px">
        <input type="checkbox" id="deleteNotes">
        Delete all Notes / Files
      </label>

      <label style="display:block;margin-bottom:10px">
        <input type="checkbox" id="resetRevision">
        Reset Revision System to default
      </label>
    </div>
...
async function performSelectiveDelete() {
  const deleteSubjects =
    document.getElementById("deleteSubjects").checked;

  const deleteNotes =
    document.getElementById("deleteNotes").checked;

  const resetRevision =
    document.getElementById("resetRevision").checked;

  if (!deleteSubjects && !deleteNotes && !resetRevision) {
  ...
  if (resetRevision) {
    state.revisionSystem =
      defaultRevisionSystem.map(rule => ({
        ...rule,
        id: uid()
      }));
  }
...
function showModal(content) {
  ...
  root.innerHTML = `
    <div class="modal-bg" onclick="modalBackground(event)">
      <div class="modal">
        ${content}
      </div>
    </div>
