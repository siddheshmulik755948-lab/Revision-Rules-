...
function openSubjectModal(subjectId = "") {
  const subject =
    subjectId ? findSubject(subjectId) : null;
...
function openChapterModal(subjectId, chapterId = "") {
  ...
  const found =
    chapterId ? findChapter(chapterId) : null;
...
function blobToURL(blob) {
  ...
  if (!(blob instanceof Blob)) {
    throw new Error("Stored object is not a Blob.");
  }
...
function showFileViewer(file, type, url) {
  ...
  if (type === "image") {
    ...
    /*
      ...
      through an object element first.

      If Android cannot render it,
      show a dedicated Open PDF button.
    */
        ...
        <object
          ...
          <div
            ...
            <p
              ...
              Your PDF is stored correctly.
            </p>

            <button
              class="btn"
              onclick="openPDFExternally('${url}')">
              Open PDF
            </button>
    ...
    content = `
      <div
        ...
        <button
          class="btn"
          onclick="openStoredFileExternally('${file.id}')">
          Open File
        </button>
  ...
  /*
    Keep URL alive while viewer is open.
    It will be released when viewer closes.
  */

  window.__revisionTrackerFileURL =
    url;
}
...
function openPDFExternally(url) {
  ...
  if (!url) {
    toast("PDF URL is unavailable.");
    return;
  }

  /*
    Android/PWA browsers sometimes block
    window.open().

    First try a normal browser navigation
    in a new tab.

    If popup is blocked, use the same tab.
  */
...
async function openStoredFileExternally(fileId) {
